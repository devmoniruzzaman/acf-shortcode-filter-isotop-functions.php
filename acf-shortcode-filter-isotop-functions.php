...
function project_shortcode_callback_func( $atts = array(), $content = '' ) {
  ...
  <button class="button is-checked" data-filter="*"> All</button>
        <?php
            ...
            $terms = get_terms(
                ...
                array(
                    'taxonomy'   => 'project-filter-category',
                    'hide_empty' => false,
                )
            );
...
if ( $the_query->have_posts() ) {
	...
	while ( $the_query->have_posts() ) {
		$the_query->the_post();
		            $terms = get_the_terms( get_the_ID(), 'project-filter-category' );
            $terms = join(' ', wp_list_pluck( $terms , 'slug') );

		echo '<div class="element-item single-project '.$terms.'">';

		if(get_field('image')){
			echo "<div class='img-area'><img src='".get_field('image')."' alt='".get_the_title()."'>
		...
		if (get_field('button')){
		echo "<a href='".get_field('button')."'>View Details</a>";
		}
		echo '</div>';
	}
...
add_shortcode( 'projec_filter', 'project_shortcode_callback_func' );